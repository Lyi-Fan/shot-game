...
export function Player() {
  ...
  const respawnTrigger = useGameStore(state => state.respawnTrigger);
  const playerSpawnPos = useGameStore(state => state.playerSpawnPos);
  ...
  useEffect(() => {
    if (body.current && respawnTrigger > 0) {
      body.current.setTranslation({ x: playerSpawnPos[0], y: playerSpawnPos[1], z: playerSpawnPos[2] }, true);
      body.current.setLinvel({ x: 0, y: 0, z: 0 }, true);
    }
  }, [respawnTrigger, playerSpawnPos]);
  ...
  const shoot = () => {
    ...
    // Origin slightly in front of camera to avoid hitting self
    const rayStart = camera.position.clone().add(raycaster.ray.direction.clone().multiplyScalar(1.0));
    const ray = new rapier.Ray(rayStart, raycaster.ray.direction);
    const hit = world.castRay(ray, MAX_LASER_DIST, false);
      ...
      if (rb && rb.userData) {
        ...
        if (name) {
          ...
          if (name.startsWith('bot-')) {
            const botPos = rb.translation();
            const isHeadshot = (hitPoint.y - botPos.y) > 1.4;
            hitEnemy(name, isHeadshot ? 3 : 1, true);
            hitEntity = true;
          } 
          // Check if it's another player (socket ID)
          else if (name !== 'player' && useGameStore.getState().otherPlayers[name]) {
            const playerPosObj = rb.translation();
            const isHeadshot = (hitPoint.y - playerPosObj.y) > 1.4;
            hitEnemy(name, isHeadshot ? 3 : 1, true);
            hitEntity = true;
          }
        }
    ...
    addLaser(startPos, endPos, '#ffaa00');
    sfx.playLaser();
  };
  ...
  useFrame((_, delta) => {
    ...
    if (!body.current || gameState !== 'playing') {
      if (useGameStore.getState().targetedEntity) {
        useGameStore.getState().setTargetedEntity(null);
      }
      return;
    }
    ...
    if (Date.now() - lastTargetTime.current > 100) {
      ...
      let targetFound: { id: string, health: number, title: string } | null = null;
      if (hit) {
        const rb = hit.collider.parent();
        if (rb && rb.userData && rb.userData.name) {
          const name = rb.userData.name as string;
          if (name.startsWith('bot-')) {
            const bot = useGameStore.getState().enemies.find(e => e.id === name);
            if (bot && bot.state === 'active') {
              targetFound = { id: name, health: bot.health, title: 'BOT ' + name.slice(4) };
            }
          } else if (name !== 'player' && useGameStore.getState().otherPlayers[name]) {
            const player = useGameStore.getState().otherPlayers[name];
            targetFound = { id: name, health: player.health, title: 'PLAYER ' + name.substring(0, 4) };
          }
        }
      }
      
      const currentTarget = useGameStore.getState().targetedEntity;
      if (!targetFound && currentTarget) {
        useGameStore.getState().setTargetedEntity(null);
      } else if (targetFound && (!currentTarget || currentTarget.id !== targetFound.id || currentTarget.health !== targetFound.health)) {
        useGameStore.getState().setTargetedEntity(targetFound);
      }
    }
    ...
    // Update local immediately for smooth minimap
    const state = useGameStore.getState();
    const posArr: [number, number, number] = [pos.x, pos.y, pos.z];
    state.currentPlayerPos = posArr;
    
    const dir = new THREE.Vector3();
    camera.getWorldDirection(dir);
    const rotY = Math.atan2(dir.x, -dir.z);
    state.currentPlayerRot = rotY;

    // Emit position to server
    const now = Date.now();
    if (now - lastEmitTime.current > 50) {
      updatePlayerPosition(posArr, rotY);
  ...
  return (
    <>
      {!isTouchDevice.current && <PointerLockControls makeDefault />}
      <RigidBody
        ref={body}
        colliders={false}
        mass={1}
